...
export const getAll = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [data, total] = await Promise.all([
            prisma.kategori_pangkat.findMany({
                where,
                skip: (p - 1) * l,
                take: l,
                orderBy: { nama: 'asc' }
            }),
            prisma.kategori_pangkat.count({ where })
        ]);
...
export const getAktif = async (req: Request, res: Response) => {
    try {
        const data = await prisma.kategori_pangkat.findMany({
            where: { status: 'Aktif' },
            orderBy: { nama: 'asc' }
        });
...
export const getById = async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
        const data = await prisma.kategori_pangkat.findUnique({
            where: { id: Number(id) }
        });
...
export const create = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const code = await ensureUniqueCode(baseCode, async (c) => {
            const existing = await prisma.kategori_pangkat.findUnique({ where: { code: c } });
            return !existing;
        });

        const data = await prisma.kategori_pangkat.create({
...
export const update = async (req: Request, res: Response) => {
    ...
    try {
        const data = await prisma.kategori_pangkat.update({
            where: { id: Number(id) },
...
export const remove = async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
        await prisma.kategori_pangkat.delete({
            where: { id: Number(id) }
        });
