...
export const create = async (req: Request, res: Response) => {
    const { nama, warna, keterangan, status } = req.body;
    if (!nama || !warna) return res.status(400).json({ message: 'Nama dan Warna wajib diisi' });

    try {
        const baseCode = generateCode(PREFIX, nama);
        const code = await ensureUniqueCode(baseCode, async (c) => {
            const existing = await prisma.tag.findUnique({ where: { code: c } });
            return !existing;
        });

        const data = await prisma.tag.create({
            data: { code, nama, warna, keterangan, status }
        });
...
export const update = async (req: Request, res: Response) => {
    ...
    try {
        const data = await prisma.tag.update({
            where: { id: Number(id) },
            data: { nama, warna, keterangan, status }
        });
        return res.json(data);
    } catch (error: any) {
        if (error.code === 'P2025') {
