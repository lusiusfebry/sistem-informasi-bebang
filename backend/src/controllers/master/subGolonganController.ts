...
export const getAll = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [data, total] = await Promise.all([
            prisma.sub_golongan.findMany({
                where,
                skip: (p - 1) * l,
                take: l,
                orderBy: { nama: 'asc' }
            }),
            prisma.sub_golongan.count({ where })
        ]);
...
export const getAktif = async (req: Request, res: Response) => {
    try {
        const data = await prisma.sub_golongan.findMany({
            where: { status: 'Aktif' },
            orderBy: { nama: 'asc' }
        });
...
export const getById = async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
        const data = await prisma.sub_golongan.findUnique({
            where: { id: Number(id) }
        });
...
export const create = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const code = await ensureUniqueCode(baseCode, async (c) => {
            const existing = await prisma.sub_golongan.findUnique({ where: { code: c } });
            return !existing;
        });

        const data = await prisma.sub_golongan.create({
...
export const update = async (req: Request, res: Response) => {
    ...
    try {
        const data = await prisma.sub_golongan.update({
            where: { id: Number(id) },
...
export const remove = async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
        await prisma.sub_golongan.delete({
            where: { id: Number(id) }
        });
