...
export const getById = async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
        const data = await prisma.golongan.findUnique({
            where: { id: Number(id) }
        });
...
export const create = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const code = await ensureUniqueCode(baseCode, async (c) => {
            const existing = await prisma.golongan.findUnique({ where: { code: c } });
            return !existing;
        });

        const data = await prisma.golongan.create({
...
export const update = async (req: Request, res: Response) => {
    ...
    try {
        const data = await prisma.golongan.update({
            where: { id: Number(id) },
